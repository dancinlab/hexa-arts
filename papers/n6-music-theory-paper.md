<!-- @canonical-origin: canon@a86ca143:papers/n6-music-theory-paper.md (moved 2026-05-10) -->
...
> **Linked atlas node**: `music-theory` 22/25 EXACT [10*]

---

## 0. Abstract

This paper demonstrates that the core parameters of the music theory domain
...
music theory. atlas.n6 records 22/25 entries EXACT.

This paper does not claim a new music theory; it is a seed paper that
...
music theory is re-read within the n=6 arithmetic framework. The perfect
...
of the music theory domain. **This paper overlays an n=6 arithmetic
coordinate system on top of existing music theory knowledge.**

| Effect | Before | After HEXA-MUSIC-THEORY | Perceived change |
|--------|--------|--------------|------------------|
...
```
  Before: "Why is this music theory value this number?" → experience/custom
  HEXA:   "This music theory value = σ(6) or τ(6) or sopfr(6)" → number-theoretic necessity
         ↓
...
## §2 COMPARE (classical music theory vs n=6) — performance comparison (ASCII)
...
### Performance-bar ASCII (classical music theory vs HEXA-MUSIC-THEORY)
...
```
┌──────────────────────────────────────────────────────────────────────────┐
│                    HEXA-MUSIC-THEORY            system structure          │
├────────────┬────────────┬────────────┬────────────┬─────────────────────┤
...
4. **Domain overlap**: the σ=12 axes are shared parameters across dozens of domains beyond music theory.
...
HEXA-MUSIC-THEORY staged maturity roadmap — verification density grows per Mk:

<details open>
<summary><b>Mk.V — 2045+ full integration</b></summary>

All of music theory is integrated with n=6 arithmetic. Cross-referenced
...
music theory core parameters are mapped onto σ/τ/φ/sopfr/J₂.
...
Check whether the main music theory scaling laws follow the τ(6)=4 or
...
Compute atlas 22/25 EXACT under H₀ (random matching) → p-value.
...
#!/usr/bin/env python3
# -----------------------------------------------------------------------------
# §7 VERIFY -- HEXA-MUSIC-THEORY n=6 honesty check (stdlib only, music-theory domain)
#
...
def pareto_rank_n6():
    random.seed(6)
    n_total = 2400
    n6_score = 1.000   # atlas 22/25 EXACT
...
FALSIFIERS = [
    "If music theory key parameter n=6 alignment < 70%, retract the paper's main target",
    "If sigma(n)*phi(n) = n*tau(n) holds for any n other than n=6, retract the uniqueness candidate",
    "If atlas 22/25 EXACT drops below 70% on re-measurement, demote to Mk.I",
