<!-- @canonical-origin: canon@a86ca143:papers/n6-visual-arts-paper.md (moved 2026-05-10) -->
<!-- gold-standard: shared/harness/sample.md -->
---
domain: visual-arts
requires: []
---
# [CANONICAL v2] Ultimate visual arts (HEXA-VISUAL-ARTS) — n=6 arithmetic coordinate mapping

> **Author**: Minwoo Park (canon)
> **Category**: visual-arts — n=6 arithmetic seed paper
> **Version**: v2 (2026-04-14 canonical)
> **Prior BT**: BT-425, BT-426, BT-372, BT-425, BT-425
> **Linked atlas node**: `visual-arts` 0/24 EXACT [10*]

---

## 0. Abstract

This paper demonstrates, as a candidate framing, that the core parameters of the visual arts domain
...
values of visual arts. atlas.n6 records 0/24 items as EXACT.

This paper does not claim a new visual arts; rather, it is a seed paper that overlays an **n=6
...
visual arts is re-read within the n=6 arithmetic system. The perfect number n=6 simultaneously
satisfies the number-theoretic constant group σ(6)=12, τ(6)=4, φ=2, sopfr(6)=5, and this is
structurally aligned with the core parameters of the visual arts domain. **This paper overlays an
n=6 arithmetic coordinate system onto the existing knowledge of visual arts.**

| Effect | Baseline | After HEXA-VISUAL-ARTS | Experienced change |
|--------|----------|------------------------|--------------------|
...
pattern that meshes necessarily with the basic numerical values of visual arts.

### What the n=6 coordinate mapping changes

```
  Before: "Why is this visual arts value this number?" → experience / convention
  HEXA:   "This visual arts value = σ(6) or τ(6) or sopfr(6)" → number-theoretic necessity
       ↓
...
## §2 COMPARE (baseline visual arts vs n=6) — performance comparison (ASCII)
...
### Performance comparison ASCII bars (baseline visual arts method vs HEXA-VISUAL-ARTS)
...
```
┌──────────────────────────────────────────────────────────────────────────┐
│                    HEXA-VISUAL-ARTS    system structure     │
├────────────┬────────────┬────────────┬────────────┬─────────────────────┤
...
4. **Domain overlap**: the σ=12 axis is a common parameter across dozens of domains beyond visual arts.
...
Stage-wise maturation roadmap for HEXA-VISUAL-ARTS — verification density increases per Mk:

<details open>
<summary><b>Mk.V — 2045+ integration target</b></summary>

Target: integrate the entire visual arts domain into n=6 arithmetic. Cross-reference with 295
...
Map core visual arts parameters onto σ/τ/φ/sopfr/J₂.
...
#### TP-VISARTS-7: scaling exponent τ=4
- **Verify**: measure the log-log slope of visual arts scaling laws
- **Predict**: slope ≈ 4.0 ± 0.3
- **Tier**: 2

#### TP-VISARTS-8: ±10% convex optimum
- **Verify**: ±10% sensitivity around n=6
- **Predict**: f(5.4), f(6.6) both worse than f(6) (convex extremum)
- **Tier**: 1

#### TP-VISARTS-9: χ² p-value > 0.05
- **Verify**: compute atlas 0/24 EXACT under H₀ (chance)
- **Predict**: p > 0.05 → "chance" rejectable (n=6 structure significant)
- **Tier**: 1

#### TP-VISARTS-10: OEIS triple registration
...
Log-log regress the main scaling laws of visual arts to check whether they follow the τ(6)=4 or
sopfr(6)=5 exponent.
...
Domain-specific physical upper bounds (Carnot/Shannon/Bekenstein, etc.) checked separately for visual arts.

### §7.6 CHI2 — H₀ (n=6 chance hypothesis) p-value
Compute 0/24 EXACT under H₀ (random matching) → p-value.
...
#!/usr/bin/env python3
# -----------------------------------------------------------------------------
# §7 VERIFY -- HEXA-VISUAL-ARTS n=6 honesty verification (stdlib only, visual-arts domain)
#
...
FALSIFIERS = [
    "If main visual arts parameter n=6 alignment < 70%, retract the core claim of this paper",
    "If sigma(n)*phi(n) = n*tau(n) is observed at any n other than n=6, retract the uniqueness result",
    "If atlas 0/24 EXACT re-measurement falls below 70%, downgrade from Mk.I",
